...
describe('StreamingRenderCoordinator', () => {
  ...
  it('flushes a new hidden stream after a cancelled render finishes without another frame', async () => {
    const oldRender = createDeferred();
    const render = jest.fn((content: string) => (
      content === 'old conversation' ? oldRender.promise : Promise.resolve()
    ));
    const coordinator = new StreamingRenderCoordinator<string>({
      render,
      getOwnerWindow: () => window,
    ...
    coordinator.request('old conversation');
    jest.advanceTimersByTime(16);
    await flushMicrotasks();

    coordinator.cancel();
    coordinator.setAvailable(false);
    coordinator.request('new conversation final');
    let flushed = false;
    ...
    expect(render).toHaveBeenLastCalledWith('new conversation final');
