...
describe('ClaudianPlugin', () => {
  ...
  describe('conversation note association migration', () => {
    ...
    it('loads deferred metadata before migrating a renamed note association', async () => {
      const deferredMetadata = {
        id: 'deferred-note-association',
        providerId: 'claude' as const,
        title: 'Deferred note association',
        createdAt: 1,
        updatedAt: 2,
        currentNote: 'notes/old.md',
      };
      const scanSpy = jest.spyOn(SessionStorage.prototype, 'scanMetadata')
        .mockResolvedValue({
          metadata: [deferredMetadata],
      ...
      await plugin.onload();
      expect(plugin.getConversationSync(deferredMetadata.id)).toBeNull();

      await getVaultEventHandler('rename')(createTFile('notes/new.md'), 'notes/old.md');

      expect(scanSpy).toHaveBeenCalledTimes(1);
      expect(plugin.getConversationSync(deferredMetadata.id)?.currentNote).toBe('notes/new.md');
      scanSpy.mockRestore();
    });
