...
export default class ClaudianPlugin extends Plugin {
  ...
  private async handleConversationNoteAssociationChange(
    ...
    await this.ensureConversationMetadataLoadedForMutation();
  ...
  private async ensureConversationMetadataLoadedForMutation(): Promise<void> {
    ...
    if (!this.remainingSessionMetadataLoad) {
      // A previous incomplete or failed scan must not leave note associations
      // stranded in metadata that has not entered the repository yet.
      this.pendingSessionMetadataScan = true;
      this.startRemainingSessionMetadataLoad();
    }
